...
def gerar_parecer(transacao):
    print("2. Gerando parecerda transação")
    prompt_sistema = f"""
...
def gerar_recomendacao(transacao):
    print("3. Gerando recomendação para transações avaliadas")
    prompt_sistema = f"""
...
for transacao in transacoes_analisadas["transacoes"]:
    if transacao["status"] == "Possível Fraude":
        ...
        recomendacao = gerar_recomendacao(transacao)
...
print("Finalizou a procedimento")
